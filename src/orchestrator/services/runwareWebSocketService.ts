...
export class RunwareWebSocketService {
  ...
  constructor() {
    ...
    this.maxConcurrentImages = parseInt(process.env.MAX_CONCURRENT_IMAGES || '5', 10);
    this.batchSize = parseInt(process.env.BATCH_SIZE_IMAGES || '5', 10);
  ...
  async generateImagesForScenes(
    ...
    logger.info('🎨 Starting batch image generation via WebSocket', {
      ...
      model,
      dimensions: `${width}x${height}`,
      maxConcurrent: this.maxConcurrentImages
    });
  ...
  private async processWithConcurrencyLimit<T, R>(
    ...
    const optimalBatchSize = this.batchSize;
