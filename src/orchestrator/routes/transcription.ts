...
import { RunPodWhisperOfficialService } from '../services/runpodWhisperOfficialService';
...
let whisperOfficialService: RunPodWhisperOfficialService | null = null;
...
function getWhisperOfficialService(): RunPodWhisperOfficialService {
  if (!whisperOfficialService) {
    whisperOfficialService = new RunPodWhisperOfficialService();
  }
  return whisperOfficialService;
}
...
// Process audio transcription with RunPod OpenAI Whisper Official
// ============================================
router.post('/runpod/audio/transcribe-whisper', authenticateApiKey, async (req: Request, res: Response) => {
  const startTime = new Date();
  const jobId = randomUUID();
  ...
  try {
    ...
    const transcriptionRequest: TranscriptionRequest = {
      ...
      // OpenAI Whisper Official doesn't use VAD
      enable_vad: undefined
    };

    console.log(`[Transcription-Whisper] Processing: ${audio_url}`);

    // Step 1: Get transcription from RunPod worker (OpenAI Whisper Official)
    const transcriptionOutput = await getWhisperOfficialService().processTranscription(transcriptionRequest);

    console.log(`[Transcription-Whisper] Job ${jobId} - Transcription completed`);
    console.log(`[Transcription-Whisper] Language detected: ${transcriptionOutput.detected_language}`);
    console.log(`[Transcription-Whisper] Segments: ${transcriptionOutput.segments.length}`);
    console.log(`[Transcription-Whisper] Words: ${transcriptionOutput.word_timestamps?.length || 0}`);
    ...
    console.log(`[Transcription-Whisper] Job ${jobId} - Files generated locally`);

    // Step 3: Generate ASS karaoke only if word timestamps are available
    let karaokeASS = '';
    if (transcriptionOutput.word_timestamps && transcriptionOutput.word_timestamps.length > 0) {
      karaokeASS = TranscriptionFormatter.toASSKaraoke(transcriptionOutput.word_timestamps);
      console.log(`[Transcription-Whisper] Job ${jobId} - ASS karaoke generated`);
    } else {
      console.warn(`[Transcription-Whisper] Job ${jobId} - No word timestamps available, skipping ASS karaoke`);
    }
    ...
    console.log(`[Transcription-Whisper] Job ${jobId} - Files uploaded to S3`);
    ...
    console.log(`[Transcription-Whisper] Job ${jobId} completed in ${response.execution.durationSeconds}s`);
    ...
    console.error(`[Transcription-Whisper] Job ${jobId} failed:`, error);
...
// Health check for OpenAI Whisper Official transcription service
// ============================================
router.get('/runpod/audio/transcribe-whisper/health', async (_req: Request, res: Response) => {
  try {
    const whisperOfficialHealth = await getWhisperOfficialService().healthCheck();

    return res.status(whisperOfficialHealth.healthy ? 200 : 503).json({
      status: whisperOfficialHealth.healthy ? 'healthy' : 'unhealthy',
      service: 'transcription-whisper-official',
      whisper: whisperOfficialHealth,
      timestamp: new Date().toISOString()
    });
  } catch (error: any) {
    return res.status(503).json({
