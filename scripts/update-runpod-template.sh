...
CREATE_RESPONSE=$(curl -s -X POST "https://api.runpod.io/graphql" \
  ...
  -H "Authorization: Bearer $API_KEY" \
  -d "{
    \"query\": \"mutation { saveTemplate(input: { name: \\\"$NEW_TEMPLATE_NAME\\\", imageName: \\\"$IMAGE_NAME\\\", dockerArgs: \\\"node dist/worker/handler.js\\\", containerDiskInGb: 10, volumeInGb: 0, isServerless: true, env: [{key: \\\"WORK_DIR\\\", value: \\\"/tmp/work\\\"}, {key: \\\"OUTPUT_DIR\\\", value: \\\"/tmp/output\\\"}, {key: \\\"NODE_ENV\\\", value: \\\"production\\\"}, {key: \\\"BATCH_SIZE\\\", value: \\\"3\\\"}] }) { id name imageName dockerArgs env { key value } } }\"
  }")
